...
def data_entry(email, password, age, birth_country, residence_country):
    conn = sqlite3.connect('users.db') #this connects to a database. If the database doesnt exist, it will create a new database and then connects to it from the second time onwards
    c = conn.cursor() #this is where the current cursor is at in the database
    c.execute("INSERT INTO users (email, psw, age, bc, rc) VALUES(?, ?, ?, ?, ?)", 
    (email, password, age, birth_country, residence_country)) #follow the same order as attributes when building the database table
